...
// [[Rcpp::export]]
List SEB(mat D, uvec times, std::string order = "successive") {
  ...
  if (nc > 1) {
    ...
    for (int i = 1; i <= nints; i++) {
      ...
      const List subLabels = SEB(subD, times.subvec(1, nc - 1));
      const uvec subLabels0 = subLabels[0];
      const mat subInts = subLabels["intervals"];
      int k = 0;
      for (int j = 0; j < nr; j++) {
        if (labelsVec[j] == i) {
          labelsMat[j] = subLabels0[k] + ncells0 * (i - 1);
          k++;
        }
      }
    ...
    Ints0.col(1) = breaks;
    return List::create(_["labels"] = labelsVec, _["intervals"] = Ints0);
  }
}

// [[Rcpp::export]]
List SEB2(mat D, uvec times, std::string order = "successive", bool intervals = true) {
  const int nr = D.n_rows;
  const int nc = D.n_cols;
  if (times.n_elem != nc)
    stop("The number of specified cutting times has to be the same as the number of columns.");
  ...
  if (nc > 1) {
    ...
    for (int i = 1; i <= nints; i++) {
      const mat subD = D1.rows(find(labelsVec == i));
      const List subLabels = SEB2(subD, times.subvec(1, nc - 1));
      const uvec subLabels0 = subLabels[0];
      const mat subInts = subLabels["intervals"];
      int k = 0;
      for (int j = 0; j < nr; j++) {
